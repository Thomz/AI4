...
int main(int argc, char **argv) {
	...
	Mat src = imread("1.png", CV_LOAD_IMAGE_UNCHANGED);
	imshow("dfs", src);
	Mat filtered;
	...
	for(int i=0; i<singleObjects.size(); i++){
		...
		vector<KeyPoint> anc_keypoints;
		detector.detect(singleObjects[i], anc_keypoints);

		// Draw keypoints on new pic

		drawKeypoints(singleObjects[i], anc_keypoints, output);
		imshow(NumberToString(10+i), output);
		output = Mat::zeros( output.size(), CV_8UC3 );
	}
